...
//				Scanner scan = new Scanner(System.in);
				inputOmokOnBoard(board, power, count, pers, ox, oy);
...
//			System.out.println("종료합니다.");
	...
	private static void quitGame(Scanner scan, int pers, boolean power) {
		System.out.println("게임을 계속 진행하시겠습니까? (0.계속 진행 그 외.종료) >"); // boolean으로 활용하는게 더 좋을 것 같음 0
		pers = scan.nextInt();
		power = (pers == 0) ? true : false; // 삼항연산자
		System.out.println("종료합니다.");
	...
	private static void inputOmokOnBoard(char[][] board, boolean power, int count, int pers, int ox, int oy) {
		while (power) {
	...
	static void initBoard(char[][] board) { // 매개변수가 참조변수(값이 아니라) - 참조변수기때문에 공유하는것
		{
			for (int y = 1; y <= 10; y++) {
				for (int x = 1; x <= 10; x++) {
					if (x == 1 && y == 1)
